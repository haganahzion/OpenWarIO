...
export class ResearchPanel extends LitElement implements Layer {
  ...
  tick() {
    ...
    if (player !== this.myPlayer) {
      ...
      this.requestUpdate();
    } else if (this.isOpen && player) {
      // Update progress while panel is open
      this.requestUpdate();
    }
  }
  ...
  render() {
    if (!this.myPlayer?.isAlive() || this.game.inSpawnPhase()) {
      return null;
    }
    ...
    return html`
      <div class="fixed bottom-4 left-4 z-1000">
        ...
        <!-- Research Panel -->
        ${this.isOpen ? html`
          <div class="absolute bottom-12 left-0 w-80 max-h-96 overflow-y-auto bg-slate-900/95 backdrop-blur-sm rounded-lg shadow-xl border border-slate-700 p-3">
